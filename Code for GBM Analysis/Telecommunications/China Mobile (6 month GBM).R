...
chl = chl[c(30:177),]
par(mfrow=c(2,2))
chl["Time Interval"] = c(1:nrow(chl))
ggplot(data=chl,aes(x=chl$"Time Interval",y=chl$Close))+
  ggtitle("China Mobile's Stock Price Trend")+
  geom_line() +xlab("Time Interval") + ylab("Closing Price")
mu_model = mean(chl$Close[1:127])
S_0 = chl$Close[127]
#We need to calculate annualized volatility for the sigma parameter
chl['Daily Change'] <- 0
for (i in 2:nrow(chl)){
  chl$'Daily Change'[i-1] = (chl$Close[i] - chl$Close[i-1])/chl$Close[i-1]
}
sd_model = sd(chl$'Daily Change'[1:127])
...
plot(seq(0,1000,by=1),S_t,type="l", main = "Geometric Brownian Motion Simulation of China Mobile's Stock Price")
for (i in 2:100){
  S_t <- GBM(S_0,r=0,sigma=sd_model,T=30,N=1000)
  lines(seq(0,1000,by=1),S_t,type="l")
  final_stock_price[i] = S_t[length(S_t)]
}
hist(final_stock_price, main = "GBM Histogram of China Mobile Stock Price with no drift")
...
chl['Daily Stock Return'] <- 0
chl$'Daily Stock Return' = (chl$Close - chl$Open)/chl$Open 
hist(chl$'Daily Stock Return'*100, freq =FALSE, main = "Histogram of China Mobile's Daily Return", xlab = "Daily Percentage Change")
xpt <- seq(-6,4,by=0.01)
ypt <- dnorm(xpt, mean(chl$'Daily Stock Return')*100, sd(chl$'Daily Stock Return')*100)
lines(xpt, ypt)
qqnorm(chl$'Daily Stock Return')
qqline(chl$'Daily Stock Return')
...
plot(seq(0,1000,by=1),S_t,type="l", main = "Geometric Brownian Motion Simulation of China Mobile's Stock Price with Updated Parameters")
for (i in 2:100){
  S_t <- GBM(S_0,r=drift,sigma=sigma_star,T=30,N=1000)
  lines(seq(0,1000,by=1),S_t,type="l")
  new_final_stock_price[i] = S_t[length(S_t)]
}
hist(new_final_stock_price, main = "GBM Histogram of China Mobile Stock Price with drift")
