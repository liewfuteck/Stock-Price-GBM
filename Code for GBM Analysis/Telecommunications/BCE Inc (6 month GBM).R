library(ggplot2)
set.seed(4238)
bce = read.csv("C:/Users/futeck/Desktop/ST4238 project/20 Companies/Telecommunications/BCE.csv", header = TRUE)
bce = bce[c(30:177),]
bce["Time Interval"] = c(1:nrow(bce))
ggplot(data=bce,aes(x=bce$"Time Interval",y=bce$Close))+
  ggtitle("BCE Inc.'s Stock Price Trend")+
  geom_line() +xlab("Time Interval") + ylab("Closing Price")
mu_model = mean(bce$Close[1:127])
S_0 = bce$Close[127]
#We need to calculate annualized volatility for the sigma parameter
bce['Daily Change'] <- 0
for (i in 2:nrow(bce)){
  bce$'Daily Change'[i-1] = (bce$Close[i] - bce$Close[i-1])/bce$Close[i-1]
}
sd_model = sd(bce$'Daily Change'[1:127])
...
plot(seq(0,1000,by=1),S_t,type="l", main = "Geometric Brownian Motion Simulation of BCE Inc.'s Stock Price")
for (i in 2:100){
  S_t <- GBM(S_0,r=0,sigma=sd_model,T=30,N=1000)
  lines(seq(0,1000,by=1),S_t,type="l")
  final_stock_price[i] = S_t[length(S_t)]
}
hist(final_stock_price, main = "GBM Histogram of BCE Inc.'s Stock Price with no drift")
...
bce['Daily Stock Return'] <- 0
bce$'Daily Stock Return' = (bce$Close - bce$Open)/bce$Open 
hist(bce$'Daily Stock Return'*100, freq =FALSE, main = "Histogram of BCE Inc.'s Daily Return", xlab = "Daily Percentage Change")
xpt <- seq(-6,4,by=0.01)
ypt <- dnorm(xpt, mean(bce$'Daily Stock Return')*100, sd(bce$'Daily Stock Return')*100)
lines(xpt, ypt)
qqnorm(bce$'Daily Stock Return')
qqline(bce$'Daily Stock Return')
...
bce['u'] <- 0
for (i in 2:127){
  bce$u[i] = log(bce$Close[i]/bce$Close[i-1]) 
}
u_bar = sum(bce$u)/126
...
plot(seq(0,1000,by=1),S_t,type="l", main = "Geometric Brownian Motion Simulation of BCE Inc.'s Stock Price with Updated Parameters")
for (i in 2:100){
  S_t <- GBM(S_0,r=drift,sigma=sigma_star,T=30,N=1000)
  lines(seq(0,1000,by=1),S_t,type="l")
  new_final_stock_price[i] = S_t[length(S_t)]
}
hist(new_final_stock_price, main = "GBM Histogram of BCE Inc.'s Stock Price with drift")
