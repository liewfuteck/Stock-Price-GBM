library(ggplot2)
set.seed(4238)
vz = read.csv("C:/Users/futeck/Desktop/ST4238 project/20 Companies/Telecommunications/VZ.csv", header = TRUE)
vz = vz[c(30:177),]
vz["Time Interval"] = c(1:nrow(vz))
par(mfrow=c(2,2))
ggplot(data=vz,aes(x=vz$"Time Interval",y=vz$Close))+
  ggtitle("Verizon Communications' Stock Price Trend")+
  geom_line() +xlab("Time Interval") + ylab("Closing Price")
mu_model = mean(vz$Close[1:127])
S_0 = vz$Close[127]
#We need to calculate annualized volatility for the sigma parameter
vz['Daily Change'] <- 0
for (i in 2:nrow(vz)){
  vz$'Daily Change'[i-1] = (vz$Close[i] - vz$Close[i-1])/vz$Close[i-1]
}
sd_model = sd(vz$'Daily Change'[1:127])
...
plot(seq(0,1000,by=1),S_t,type="l", main = "Geometric Brownian Motion Simulation of Verizon Communications' Stock Price")
for (i in 2:100){
  S_t <- GBM(S_0,r=0,sigma=sd_model,T=30,N=1000)
  lines(seq(0,1000,by=1),S_t,type="l")
  final_stock_price[i] = S_t[length(S_t)]
}
hist(final_stock_price, main = "GBM Histogram of Verizon Communications' Stock Price with no drift")
...
vz['Daily Stock Return'] <- 0
vz$'Daily Stock Return' = (vz$Close - vz$Open)/vz$Open 
hist(vz$'Daily Stock Return'*100, freq =FALSE, main = "Histogram of Verizon Communications' Daily Return", xlab = "Daily Percentage Change")
xpt <- seq(-6,4,by=0.01)
ypt <- dnorm(xpt, mean(vz$'Daily Stock Return')*100, sd(vz$'Daily Stock Return')*100)
lines(xpt, ypt)
qqnorm(vz$'Daily Stock Return')
qqline(vz$'Daily Stock Return')
...
vz['u'] <- 0
for (i in 2:127){
  vz$u[i] = log(vz$Close[i]/vz$Close[i-1]) 
}
u_bar = sum(vz$u)/126
...
plot(seq(0,1000,by=1),S_t,type="l", main = "Geometric Brownian Motion Simulation of Verizon Communications' Stock Price with Updated Parameters")
for (i in 2:100){
  S_t <- GBM(S_0,r=drift,sigma=sigma_star,T=30,N=1000)
  lines(seq(0,1000,by=1),S_t,type="l")
  new_final_stock_price[i] = S_t[length(S_t)]
}
hist(new_final_stock_price,  main = "GBM Histogram of Verizon Communications' Stock Price with drift")
