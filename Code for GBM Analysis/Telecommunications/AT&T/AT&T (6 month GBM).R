library(ggplot2)
set.seed(4238)
atnt = read.csv("C:/Users/futeck/Desktop/ST4238 project/20 Companies/Telecommunications/T.csv", header = TRUE)
atnt = atnt[c(30:177),]
par(mfrow=c(2,2))
atnt["Time Interval"] = c(1:nrow(atnt))
ggplot(data=atnt,aes(x=atnt$"Time Interval",y=atnt$Close))+
  ggtitle("AT&T's Stock Price Trend")+
  geom_line() +xlab("Time Interval") + ylab("Closing Price")
mu_model = mean(atnt$Close[1:127])
S_0 = atnt$Close[127]
#We need to calculate annualized volatility for the sigma parameter
atnt['Daily Change'] <- 0
for (i in 2:nrow(atnt)){
  atnt$'Daily Change'[i-1] = (atnt$Close[i] - atnt$Close[i-1])/atnt$Close[i-1]
}
sd_model = sd(atnt$'Daily Change'[1:127])
...
plot(seq(0,1000,by=1),S_t,type="l", main = "Geometric Brownian Motion Simulation of AT&T's Stock Price")
for (i in 2:100){
  S_t <- GBM(S_0,r=0,sigma=sd_model,T=30,N=1000)
  lines(seq(0,1000,by=1),S_t,type="l")
  final_stock_price[i] = S_t[length(S_t)]
}
hist(final_stock_price, main = "GBM Histogram of AT&T's Stock Price with no drift")
...
atnt['Daily Stock Return'] <- 0
atnt$'Daily Stock Return' = (atnt$Close - atnt$Open)/atnt$Open 
hist(atnt$'Daily Stock Return'*100, freq =FALSE, main = "Histogram of AT&T's Daily Return", xlab = "Daily Percentage Change")
xpt <- seq(-6,4,by=0.01)
ypt <- dnorm(xpt, mean(atnt$'Daily Stock Return')*100, sd(atnt$'Daily Stock Return')*100)
lines(xpt, ypt)
qqnorm(atnt$'Daily Stock Return')
qqline(atnt$'Daily Stock Return')
...
atnt['u'] <- 0
for (i in 2:127){
  atnt$u[i] = log(atnt$Close[i]/atnt$Close[i-1]) 
}
u_bar = sum(atnt$u)/126

v = numeric(126)
for (i in 2:127){
  v[i-1] = atnt$u[i] - u_bar
}
v = sqrt(sum(v^2)/125)
...
plot(seq(0,1000,by=1),S_t,type="l", main = "Geometric Brownian Motion Simulation of AT&T's Stock Price with Updated Parameters")
for (i in 2:100){
  S_t <- GBM(S_0,r=drift,sigma=sigma_star,T=30,N=1000)
  lines(seq(0,1000,by=1),S_t,type="l")
  new_final_stock_price[i] = S_t[length(S_t)]
}
hist(new_final_stock_price, main = "GBM Histogram of AT&T's Stock Price with drift")
