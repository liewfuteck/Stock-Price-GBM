library(ggplot2)
set.seed(4238)
jnj = read.csv("C:/Users/futeck/Desktop/ST4238 project/20 Companies/Healthcare/JNJ.csv", header = TRUE)
jnj = jnj[c(30:177),]
par(mfrow=c(2,2))
jnj["Time Interval"] = c(1:nrow(jnj))
ggplot(data=jnj,aes(x=jnj$"Time Interval",y=jnj$Close))+
  ggtitle("Johnson & Johnson Stock Price Trend")+
  geom_line() +xlab("Time Interval") + ylab("Closing Price")
mu_model = mean(jnj$Close[1:127])
S_0 = jnj$Close[127]
#We need to calculate annualized volatility for the sigma parameter
jnj['Daily Change'] <- 0
for (i in 2:nrow(jnj)){
  jnj$'Daily Change'[i-1] = (jnj$Close[i] - jnj$Close[i-1])/jnj$Close[i-1]
}
sd_model = sd(jnj$'Daily Change'[1:127])
...
plot(seq(0,1000,by=1),S_t,type="l", main = "Geometric Brownian Motion Simulation of Johnson & Johnson Stock Price")
for (i in 2:100){
  S_t <- GBM(S_0,r=0,sigma=sd_model,T=30,N=1000)
  lines(seq(0,1000,by=1),S_t,type="l")
  final_stock_price[i] = S_t[length(S_t)]
}
hist(final_stock_price, main = "GBM Histogram of Johnson & Johnson Stock Price with no drift")
...
jnj['Daily Stock Return'] <- 0
jnj$'Daily Stock Return' = (jnj$Close - jnj$Open)/jnj$Open 
hist(jnj$'Daily Stock Return'*100, freq =FALSE, main = "Histogram of Johnson & Johnson's Daily Return", xlab = "Daily Percentage Change")
xpt <- seq(-6,4,by=0.01)
ypt <- dnorm(xpt, mean(jnj$'Daily Stock Return')*100, sd(jnj$'Daily Stock Return')*100)
lines(xpt, ypt)
qqnorm(jnj$'Daily Stock Return')
qqline(jnj$'Daily Stock Return')
...
jnj['u'] <- 0
for (i in 2:127){
  jnj$u[i] = log(jnj$Close[i]/jnj$Close[i-1]) 
}
u_bar = sum(jnj$u)/126
...
plot(seq(0,1000,by=1),S_t,type="l", main = "Geometric Brownian Motion Simulation of Johnson & Johnson's Stock Price with Updated Parameters")
for (i in 2:100){
  S_t <- GBM(S_0,r=drift,sigma=sigma_star,T=30,N=1000)
  lines(seq(0,1000,by=1),S_t,type="l")
  new_final_stock_price[i] = S_t[length(S_t)]
}
hist(new_final_stock_price, main = "GBM Histogram of Johnson & Johnson Stock Price with drift")
