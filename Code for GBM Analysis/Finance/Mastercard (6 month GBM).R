...
ggplot(data=ma,aes(x=ma$"Time Interval",y=ma$Close))+
  ggtitle("Mastercard's Stock Price Trend")+
  geom_line() +xlab("Time Interval") + ylab("Closing Price")
mu_model = mean(ma$Close[1:127])
S_0 = ma$Close[127]
#We need to calculate annualized volatility for the sigma parameter
ma['Daily Change'] <- 0
for (i in 2:nrow(ma)){
  ma$'Daily Change'[i-1] = (ma$Close[i] - ma$Close[i-1])/ma$Close[i-1]
}
sd_model = sd(ma$'Daily Change'[1:127])
...
plot(seq(0,1000,by=1),S_t,type="l", main = "Geometric Brownian Motion Simulation of Mastercard's Stock Price")
for (i in 2:100){
  S_t <- GBM(S_0,r=0,sigma=sd_model,T=30,N=1000)
  lines(seq(0,1000,by=1),S_t,type="l")
  final_stock_price[i] = S_t[length(S_t)]
}
hist(final_stock_price, main = "GBM Histogram of Mastercard Stock Price with no drift")
...
ma['Daily Stock Return'] <- 0
ma$'Daily Stock Return' = (ma$Close - ma$Open)/ma$Open 
hist(ma$'Daily Stock Return'*100, freq =FALSE, main = "Histogram of Mastercard's Daily Return", xlab = "Daily Percentage Change")
xpt <- seq(-6,4,by=0.01)
ypt <- dnorm(xpt, mean(ma$'Daily Stock Return')*100, sd(ma$'Daily Stock Return')*100)
lines(xpt, ypt)
qqnorm(ma$'Daily Stock Return')
qqline(ma$'Daily Stock Return')
...
plot(seq(0,1000,by=1),S_t,type="l", main = "Geometric Brownian Motion Simulation of Mastercard's Stock Price with Updated Parameters")
for (i in 2:100){
  S_t <- GBM(S_0,r=drift,sigma=sigma_star,T=30,N=1000)
  lines(seq(0,1000,by=1),S_t,type="l")
  new_final_stock_price[i] = S_t[length(S_t)]
}
hist(new_final_stock_price, main = "GBM Histogram of Mastercard Stock Price with drift")
