library(ggplot2)
set.seed(4238)
jpm = read.csv("C:/Users/futeck/Desktop/ST4238 project/20 Companies/Finance/JPM.csv", header = TRUE)
jpm = jpm[c(30:177),]
par(mfrow=c(2,2))
jpm["Time Interval"] = c(1:nrow(jpm))
ggplot(data=jpm,aes(x=jpm$"Time Interval",y=jpm$Close))+
  ggtitle("JP Morgan's Stock Price Trend")+
  geom_line() +xlab("Time Interval") + ylab("Closing Price")
mu_model = mean(jpm$Close[1:127])
S_0 = jpm$Close[127]
#We need to calculate annualized volatility for the sigma parameter
jpm['Daily Change'] <- 0
for (i in 2:nrow(jpm)){
  jpm$'Daily Change'[i-1] = (jpm$Close[i] - jpm$Close[i-1])/jpm$Close[i-1]
}
sd_model = sd(jpm$'Daily Change'[1:127])
...
plot(seq(0,1000,by=1),S_t,type="l", main = "Geometric Brownian Motion Simulation of JP Morgan's Stock Price")
for (i in 2:100){
  S_t <- GBM(S_0,r=0,sigma=sd_model,T=30,N=1000)
  lines(seq(0,1000,by=1),S_t,type="l")
  final_stock_price[i] = S_t[length(S_t)]
}
hist(final_stock_price, main = "GBM Histogram of JP Morgan Stock Price with no drift")
...
jpm['Daily Stock Return'] <- 0
jpm$'Daily Stock Return' = (jpm$Close - jpm$Open)/jpm$Open 
hist(jpm$'Daily Stock Return'*100, freq =FALSE, main = "Histogram of JP Morgan's Daily Return", xlab = "Daily Percentage Change")
xpt <- seq(-3,3,by=0.01)
ypt <- dnorm(xpt, mean(jpm$'Daily Stock Return')*100, sd(jpm$'Daily Stock Return')*100)
lines(xpt, ypt)
qqnorm(jpm$'Daily Stock Return')
qqline(jpm$'Daily Stock Return')
...
jpm['u'] <- 0
for (i in 2:127){
  jpm$u[i] = log(jpm$Close[i]/jpm$Close[i-1]) 
}
u_bar = sum(jpm$u)/126
...
plot(seq(0,1000,by=1),S_t,type="l", main = "Geometric Brownian Motion Simulation of JP Morgan's Stock Price with Updated Parameters")
for (i in 2:100){
  S_t <- GBM(S_0,r=drift,sigma=sigma_star,T=30,N=1000)
  lines(seq(0,1000,by=1),S_t,type="l")
  new_final_stock_price[i] = S_t[length(S_t)]
}
hist(new_final_stock_price, main = "GBM Histogram of JP Morgan Stock Price with drift")
