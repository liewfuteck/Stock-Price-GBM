library(ggplot2)
set.seed(4238)
png = read.csv("C:/Users/futeck/Desktop/ST4238 project/20 Companies/Consumer Services/PG.csv", header = TRUE)
png = png[c(30:177),]
par(mfrow=c(2,2))
png["Time Interval"] = c(1:nrow(png))
ggplot(data=png,aes(x=png$"Time Interval",y=png$Close))+
  ggtitle("Proctor & Gamble's Stock Price Trend")+
  geom_line() +xlab("Time Interval") + ylab("Closing Price")
mu_model = mean(png$Close[1:127])
S_0 = png$Close[127]
#We need to calculate annualized volatility for the sigma parameter
png['Daily Change'] <- 0
for (i in 2:nrow(png)){
  png$'Daily Change'[i-1] = (png$Close[i] - png$Close[i-1])/png$Close[i-1]
}
sd_model = sd(png$'Daily Change'[1:127])
...
plot(seq(0,1000,by=1),S_t,type="l", main = "Geometric Brownian Motion Simulation of Proctor & Gamble's Stock Price")
for (i in 2:100){
  S_t <- GBM(S_0,r=0,sigma=sd_model,T=30,N=1000)
  lines(seq(0,1000,by=1),S_t,type="l")
  final_stock_price[i] = S_t[length(S_t)]
}
hist(final_stock_price, main = "GBM Histogram of P&G's Stock Price with no drift")
...
png['Daily Stock Return'] <- 0
png$'Daily Stock Return' = (png$Close - png$Open)/png$Open 
hist(png$'Daily Stock Return'*100, freq =FALSE, main = "Histogram of Proctor & Gamble's Daily Return", xlab = "Daily Percentage Change")
xpt <- seq(-6,4,by=0.01)
ypt <- dnorm(xpt, mean(png$'Daily Stock Return')*100, sd(png$'Daily Stock Return')*100)
lines(xpt, ypt)
qqnorm(png$'Daily Stock Return')
qqline(png$'Daily Stock Return')
...
png['u'] <- 0
for (i in 2:127){
  png$u[i] = log(png$Close[i]/png$Close[i-1]) 
}
u_bar = sum(png$u)/126
...
plot(seq(0,1000,by=1),S_t,type="l", main = "Geometric Brownian Motion Simulation of Proctor & Gamble's Stock Price with Updated Parameters")
for (i in 2:100){
  S_t <- GBM(S_0,r=drift,sigma=sigma_star,T=30,N=1000)
  lines(seq(0,1000,by=1),S_t,type="l")
  new_final_stock_price[i] = S_t[length(S_t)]
}
hist(new_final_stock_price, main = "GBM Histogram of P&G's Stock Price with drift")
