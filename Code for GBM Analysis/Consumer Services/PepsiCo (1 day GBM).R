...
pepsi = pepsi[c(27:177),]
pepsi["Time Interval"] = c(1:nrow(pepsi))

S_0 = pepsi$Close[nrow(pepsi)-1]
...
plot(seq(0,20,by=1),S_t,type="l", main = "Geometric Brownian Motion Simulation of PepsiCo's Stock Price with Updated Parameters")
for (i in 2:100){
  S_t <- GBM(S_0,r=drift,sigma=sigma_star,T=1,N=20)
  lines(seq(0,20,by=1),S_t,type="l")
  new_final_stock_price[i] = S_t[length(S_t)]
}
hist(new_final_stock_price, main = "1-day GBM Simulation Histogram of PepsiCo Stock Price with drift")
