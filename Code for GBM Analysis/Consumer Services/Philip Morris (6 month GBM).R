library(ggplot2)
set.seed(4238)
pm = read.csv("C:/Users/futeck/Desktop/ST4238 project/20 Companies/Consumer Services/PM.csv", header = TRUE)
pm = pm[c(30:177),]
par(mfrow=c(2,2))
pm["Time Interval"] = c(1:nrow(pm))
ggplot(data=pm,aes(x=pm$"Time Interval",y=pm$Close))+
  ggtitle("Philip Morris' Stock Price Trend")+
  geom_line() +xlab("Time Interval") + ylab("Closing Price")
mu_model = mean(pm$Close[1:127])
S_0 = pm$Close[127]
#We need to calculate annualized volatility for the sigma parameter
pm['Daily Change'] <- 0
for (i in 2:nrow(pm)){
  pm$'Daily Change'[i-1] = (pm$Close[i] - pm$Close[i-1])/pm$Close[i-1]
}
sd_model = sd(pm$'Daily Change'[1:127])
...
plot(seq(0,1000,by=1),S_t,type="l", main = "Geometric Brownian Motion Simulation of Philip Morris' Stock Price")
for (i in 2:100){
  S_t <- GBM(S_0,r=0,sigma=sd_model,T=30,N=1000)
  lines(seq(0,1000,by=1),S_t,type="l")
  final_stock_price[i] = S_t[length(S_t)]
}
hist(final_stock_price, main = "GBM Histogram of Philip Morris' Stock Price with no drift")
...
pm['Daily Stock Return'] <- 0
pm$'Daily Stock Return' = (pm$Close - pm$Open)/pm$Open 
hist(pm$'Daily Stock Return'*100, freq =FALSE, main = "Histogram of Philip Morris' Daily Return", xlab = "Daily Percentage Change")
xpt <- seq(-6,4,by=0.01)
ypt <- dnorm(xpt, mean(pm$'Daily Stock Return')*100, sd(pm$'Daily Stock Return')*100)
lines(xpt, ypt)
qqnorm(pm$'Daily Stock Return')
qqline(pm$'Daily Stock Return')
...
pm['u'] <- 0
for (i in 2:127){
  pm$u[i] = log(pm$Close[i]/pm$Close[i-1]) 
}
u_bar = sum(pm$u)/126
...
plot(seq(0,1000,by=1),S_t,type="l", main = "Geometric Brownian Motion Simulation of Philip Morris' Stock Price with Updated Parameters")
for (i in 2:100){
  S_t <- GBM(S_0,r=drift,sigma=sigma_star,T=30,N=1000)
  lines(seq(0,1000,by=1),S_t,type="l")
  new_final_stock_price[i] = S_t[length(S_t)]
}
hist(new_final_stock_price, main = "GBM Histogram of Philip Morris' Stock Price with drift")
